% round3.m
% 计算第三回合所有可能的牌堆分布及每种牌堆下的胜率/统计，结果保存为 CSV
% 运行：在 MATLAB 中切换到脚本目录，运行： round3
...
% 直接枚举第三回合的所有可能牌堆（和为16，且每项在0..4）
fprintf('Enumerating all third-round pile configurations (sum=16, 0..4 each)...\n');
total = 16; num_values = nRanks;
piles = [];
for c1 = 0:4
    for c2 = 0:4
        for c3 = 0:4
...
deckStates = piles;
nSecond = size(deckStates,1);
fprintf('Found %d third-round piles.\n', nSecond);

% 计算每个牌堆在原始32张牌中被留下的概率： P(c) = prod_i nchoosek(4, c_i) / nchoosek(32,16)
denomAll = nchoosek(32,16);
deckProb = zeros(nSecond,1);
for i = 1:nSecond
    ci = deckStates(i,:);
    mult = 1;
    for r = 1:num_values
...
% Step 3: 对每个第三回合牌堆计算胜率（和 round2 类似）
fprintf('Step 3: 计算每个第三回合牌堆下的发牌胜率分布并保存结果...\n');
results = struct(); results.deck = cell(nSecond,1); results.prob_from_prior = deckProb;
results.win_X = zeros(nSecond,1); results.win_Y = zeros(nSecond,1); results.win_Z = zeros(nSecond,1);
results.twoFarmers = zeros(nSecond,1); results.threeFarmers = zeros(nSecond,1);
results.farmerWinPair_X = zeros(nSecond,1); results.farmerWinPair_Y = zeros(nSecond,1); results.farmerWinPair_Z = zeros(nSecond,1);
results.farmerWinNonPair_X = zeros(nSecond,1); results.farmerWinNonPair_Y = zeros(nSecond,1); results.farmerWinNonPair_Z = zeros(nSecond,1);
results.totalDeals_next = zeros(nSecond,1);
% joint distribution storage (27 combos)
results.jointWeights = zeros(nSecond, 27);
results.jointLabels = cell(27,1);

remTotal = 16; % 第三回合牌堆总牌数
...
for idx = 1:nSecond
        ...
        for iX = 1:nComb
            ...
            for iY = 1:nComb
                ...
                for iZ = 1:nComb
                    ...
                    w3 = 1;
                    for r = 1:nRanks
                        w3 = w3 * (fact(rem(r)+1) / (fact(rem(r) - t(r)+1) * fact(aM(r)+1) * fact(aX(r)+1) * fact(aY(r)+1) * fact(aZ(r)+1)));
                    end
                    totalW = totalW + w3;
                    winX = (pX > pM); winY = (pY > pM); winZ = (pZ > pM);
                    if winX
                        fw(1) = fw(1) + w3; if isPairX, fwPair(1)=fwPair(1)+w3; else fwNon(1)=fwNon(1)+w3; end
                    end
                    ...
                    jRow(ind) = jRow(ind) + w3;
                    if winY
                        fw(2) = fw(2) + w3; if isPairY, fwPair(2)=fwPair(2)+w3; else fwNon(2)=fwNon(2)+w3; end
                    end
                    if winZ
                        fw(3) = fw(3) + w3; if isPairZ, fwPair(3)=fwPair(3)+w3; else fwNon(3)=fwNon(3)+w3; end
                    end
                    nW = winX + winY + winZ;
                    if nW >= 2, twoW = twoW + w3; end
                    if nW == 3, threeW = threeW + w3; end
                end
            end
        end
    end
    ...
    if totalDeals_next == 0
        ...
        results.twoFarmers(idx)=NaN; results.threeFarmers(idx)=NaN;
    else
    ...
    fprintf(' processed %d / %d (%.1fs)\n', idx, nSecond, elapsed_iter);
end

% 保存 CSV，参考 round1/round2 格式
fprintf('Saving round3_results.csv...\n');
...
outfn = fullfile(pwd, 'round3_results.csv');
writetable(T, outfn);
fprintf('Saved %s\n', outfn);
